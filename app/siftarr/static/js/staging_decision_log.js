(function () {
  ...
  function render(data) {
    status.textContent = data.total ? `Page ${data.page} · ${data.total} entries` : 'No decision-log entries match these filters.';
    prev.disabled = data.page <= 1;
    next.disabled = !data.has_next;
    if (!data.items.length) { results.innerHTML = '<div class="rounded-xl border border-gray-800 p-4 text-sm text-gray-400">No entries found.</div>'; return; }
    results.innerHTML = data.items.map((entry) => {
  ...
  async function load() {
    const p = params(); syncUrl(p); status.textContent = 'Loading…'; results.innerHTML = '';
    try { const r = await fetch(`/staged/decision-log?${p.toString()}`, { credentials: 'same-origin' }); if (!r.ok) throw new Error(`HTTP ${r.status}`); render(await r.json()); }
    catch (e) { status.textContent = `Error loading decision log: ${e.message}`; }
  }
  fillForm(); form.addEventListener('submit', (e) => { e.preventDefault(); page = 1; load(); });
  reset.addEventListener('click', () => { form.reset(); page = 1; load(); });
