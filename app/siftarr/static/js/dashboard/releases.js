...
function renderMovieSearchLoadingState() {
  return (
    '<div class="dashboard-details-search-loading" role="status" aria-live="polite">' +
...
function releaseAnnotationTone(release, field) {
  ...
  if (
    relevantMatches.some((match) => match.matched === true && String(match.effect || '').toLowerCase() === 'disallow')
  ) {
    return 'text-red-400';
  }
  if (
    ...
    relevantMatches.some(
      (match) => match.matched === false && String(match.effect || '').toLowerCase() === 'size_limit',
    )
  ) {
    return 'text-red-400';
  }
  if (
    relevantMatches.some(
      (match) => match.matched === true && ['allow', 'size_limit'].includes(String(match.effect || '').toLowerCase()),
    )
  ) {
  ...
  const isSizeFailure =
    !release.passed &&
    typeof release.rejection_reason === 'string' &&
    release.rejection_reason.toLowerCase().startsWith('size ');
  if (field === 'size') {
...
const RELEASE_ACTION_ICONS = {
  stage:
    '<svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 14h4l2 3h4l2-3h4v5H4zM12 4v9m-4-4 4 4 4-4"/></svg>',
  approve:
    '<svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v11m-4-4 4 4 4-4M5 19h14"/></svg>',
  reject:
  ...
  search:
    '<svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><circle cx="10.5" cy="10.5" r="6.5" stroke-width="2"/><path stroke-linecap="round" stroke-width="2" d="m15.5 15.5 4 4"/></svg>',
};
...
const RELEASE_ACTION_CLASSES = {
  ...
  approve:
    'border-emerald-500/60 bg-emerald-950/40 text-emerald-300 hover:border-emerald-400 hover:bg-emerald-600 hover:text-white',
  reject: 'border-red-500/60 bg-red-950/40 text-red-300 hover:border-red-400 hover:bg-red-700 hover:text-white',
  search:
    'border-violet-500/60 bg-violet-950/40 text-violet-300 hover:border-violet-400 hover:bg-violet-700 hover:text-white',
};
...
function renderReleaseCard(release, requestId, options = {}) {
  ...
  const isActiveSelection =
    !!release.is_active_selection ||
    !!(
      !isScopedEpisodeRelease &&
      hasActiveStagedSelection &&
      activeStagedTorrent &&
      release.title === activeStagedTorrent.title
    );
  ...
  const RELEASE_TITLE_CLASS =
    'text-[13px] leading-snug text-white font-medium overflow-wrap-anywhere line-clamp-2 lg:line-clamp-none lg:truncate';
  const titleHtml = stagedBadge
    ? '<div class="flex items-center gap-2 flex-wrap"><span class="' +
      RELEASE_TITLE_CLASS +
      '">' +
      titleText +
      '</span>' +
  ...
  const metaParts = [
    renderAnnotation(
      release.resolution,
      releaseAnnotationTone(release, 'resolution'),
      'data-release-resolution="true"',
    ),
  ...
  const metaHtml = metaParts.length
    ? '<div class="text-[11px] leading-tight text-gray-400 flex items-center gap-1 lg:gap-1.5 flex-wrap">' +
      metaParts.join('<span class="text-gray-600">·</span>') +
      '</div>'
    : '';
  ...
  const bodyHtml =
    '<div class="min-w-0 flex-1 basis-[70%] lg:basis-auto">' + titleHtml + metaHtml + coverageHtml + '</div>';
  ...
  if (activeSelectionMode && activeStagedTorrent && activeStagedTorrent.id) {
    ...
    actionHtml =
      ...
      renderIconAction(
        'approve',
        'Approve and download now',
        ' onclick="inlineStagedAction(\'/staged/' + stagedId + '/approve\', this)"',
      ) +
      renderIconAction(
        'reject',
        'Reject staged release',
        ' onclick="inlineStagedAction(\'/staged/' + stagedId + '/discard\', this)"',
      ) +
      '</div>';
  } else {
    ...
    const rejectLabel = rejected
      ...
      : storedReleaseId
        ? 'Reject this release'
        : 'Only stored releases can be rejected';
    actionHtml =
      '<div class="ml-auto flex items-center gap-1.5 shrink-0">' +
      renderIconAction(
        'stage',
        stageLabel,
        releaseUseAttrs + ' onclick="stageRelease(this)"',
        disableAction || !window.siftarrStagingModeEnabled,
      ) +
      renderIconAction(
        'approve',
        disableAction ? 'No download source available' : 'Approve and download now',
        releaseUseAttrs + ' onclick="approveRelease(this)"',
        disableAction,
      ) +
      renderIconAction(
        'reject',
        rejectLabel,
        rejectDisabled ? '' : ' onclick="rejectRelease(this, ' + requestId + ', ' + storedReleaseId + ')"',
        rejectDisabled,
      ) +
      '</div>';
  }
  ...
  if (bucket) {
    ...
    return (
      ...
      outerClass +
      (rejected ? ' opacity-60' : '') +
      ' px-3 py-2 flex flex-wrap items-start gap-3 lg:flex-nowrap lg:items-center lg:gap-3.5">' +
      scoreGutter +
      bodyHtml +
      actionHtml +
      '</div>'
  ...
  const outerClass = activeSelectionMode ? 'bg-cyan-950/20 border-b border-gray-700/60' : 'hover:bg-surface-850/60';
  return (
    '<li class="flex flex-wrap items-start gap-3 px-3 py-3 lg:flex-nowrap lg:items-center lg:gap-4 lg:px-4 ' +
    outerClass +
    (rejected ? ' opacity-60' : '') +
    '">' +
    scoreGutter +
...
function renderCoverageBadge(release) {
  ...
  const coverageText = coveredSeasons.length ? `S${coveredSeasons.join(', S')}` : 'Season coverage unknown';
  ...
  const seriesBadge =
    release.is_complete_series || release.covers_all_known_seasons
      ? '<span class="badge badge-green">Complete series</span>'
      : '';
  const sizePerSeason = release.size_per_season
    ? '<span data-release-size-per-season="true" class="' +
      (release.size_per_season_passed === true
        ? 'text-emerald-400'
        : release.size_per_season_passed === false
          ? 'text-red-400'
          : 'text-gray-400') +
      '">' +
...
const SEASON_PACK_ICON_SVG =
  '<svg class="w-3.5 h-3.5 text-brand-400 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"/></svg>';

function isMultiSeasonRelease(release) {
  const scope = release.target_scope || {};
  const coveredSeasons = Array.isArray(release.covered_seasons) ? release.covered_seasons : [];
  return (
    scope.type === 'multi_season_pack' ||
    scope.type === 'complete_series' ||
    !!release.is_complete_series ||
    coveredSeasons.length > 1
  );
}
...
function renderSeasonPackSearchButton(requestId, season) {
  ...
  return (
    '<button type="button" onclick="searchSeasonPacks(' +
    requestId +
    ', ' +
    season.season_number +
    '); event.preventDefault(); event.stopPropagation();" class="' +
    SEASON_PACK_SEARCH_BUTTON_CLASS +
    '">Search packs</button>'
  );
}
...
function findDetailsSeason(requestId, seasonNumber) {
  ...
  return (
    data.tv_info.seasons.find(function (s) {
...
const SEASON_PACK_SEARCH_BUTTON_CLASS =
  'shrink-0 rounded-lg bg-brand-600 hover:bg-brand-500 px-2.5 py-1 text-[11px] font-semibold text-white';
const SEASON_PACK_EMPTY_MESSAGE =
  '<div class="text-gray-500 text-sm py-2">No cached season-pack results yet. Search to fetch fresh results from your indexers.</div>';

function renderSeasonPackRows(releases, requestId, season) {
  return releases.length
    ? releases
        .map(function (r) {
          return renderPackRow(r, requestId, season);
        })
        .join('')
...
function stagedPacksMissingFromRows(stagedPacks, releases) {
  ...
  return (stagedPacks || []).filter(function (staged) {
    return !cachedTitles.has(String(staged.title || ''));
  });
}
...
function renderStagedPackRow(staged, _requestId) {
  const actions = staged.id
    ? '<div class="ml-auto flex items-center gap-1.5 shrink-0">' +
      ...
      renderIconAction(
        'approve',
        'Approve and download now',
        ' onclick="inlineStagedAction(\'/staged/' +
          staged.id +
          '/approve\', this); event.preventDefault(); event.stopPropagation();"',
      ) +
      renderIconAction(
        'reject',
        'Reject staged release',
        ' onclick="inlineStagedAction(\'/staged/' +
          staged.id +
          '/discard\', this); event.preventDefault(); event.stopPropagation();"',
      ) +
      '</div>'
    : '';
  return (
    '<div class="rounded-xl bg-surface-950 px-3 py-2 flex flex-wrap items-start gap-3 lg:flex-nowrap lg:items-center lg:gap-3.5" data-staged-pack-row="true">' +
...
function renderStagedPackRows(stagedPacks, requestId) {
  return (stagedPacks || [])
    .map(function (staged) {
      return renderStagedPackRow(staged, requestId);
    })
    .join('');
}
...
function renderSeasonPacksDrawer(requestId, season, seasonPacks, stagedPacks) {
  ...
  return (
    '<details id="season-packs-details-' +
    requestId +
    '-' +
    seasonNumber +
    '" class="group rounded-xl border border-gray-700/60 bg-surface-900/60" ontoggle="window.updateTvAccordionControls && window.updateTvAccordionControls()"' +
    openAttr +
    '>' +
    ...
    '</div>' +
    '</summary>' +
    '<div class="px-3 pb-3 pt-0.5 space-y-1.5" data-season-pack-results="' +
    requestId +
    '-' +
    seasonNumber +
    '">' +
...
function renderSeasonPackGroup(requestId, season, seasonPacks, stagedPacks) {
  ...
  return (
    '<div class="rounded-xl border border-gray-700/60 bg-surface-850 overflow-hidden">' +
    ...
    '<span class="text-white font-medium text-sm">Season ' +
    seasonNumber +
    '</span>' +
    ...
    '</div>' +
    '</div>' +
    '<div class="p-2.5 space-y-1.5" data-season-pack-results="' +
    requestId +
    '-' +
    seasonNumber +
    '">' +
...
function renderMultiSeasonPackGroup(requestId, multiSeasonReleases, stagedPacks) {
  ...
  countParts.push(
    multiSeasonReleases.length
      ? multiSeasonReleases.length +
          ' pack' +
          (multiSeasonReleases.length === 1 ? '' : 's') +
  ...
  const rows = multiSeasonReleases.length
    ? multiSeasonReleases
        .map(function (r) {
          return renderPackRow(r, requestId, null);
        })
        .join('')
    : orphanStaged.length
      ? ''
  ...
  return (
    ...
    ')" class="' +
    SEASON_PACK_SEARCH_BUTTON_CLASS +
    '">Search multi-season</button>' +
    '</div>' +
    '</div>' +
    '<div class="p-2.5 space-y-2" data-multi-season-pack-results="' +
    requestId +
    '">' +
    renderStagedPackRows(orphanStaged, requestId) +
    rows +
...
function stagedTorrentForEpisode(stagedTorrents, seasonNumber, episodeNumber) {
  return (
    (stagedTorrents || []).find(function (torrent) {
      const scope = torrent.target_scope || {};
...
const TV_ACCORDION_TOGGLE_CLASS =
  'tv-accordion-toggle inline-flex items-center justify-center rounded-md border border-gray-600/80 bg-surface-900/70 px-2.5 py-1 text-xs font-medium leading-4 text-gray-200 shadow-sm transition-colors hover:border-brand-400/70 hover:bg-surface-800 hover:text-white focus:outline-none focus:ring-2 focus:ring-brand-400/60 focus:ring-offset-2 focus:ring-offset-surface-900';

function renderTvAccordionToggle(scope, requestId, seasonNumber = null) {
  const toggleAttr = scope === 'season' ? 'data-tv-accordion-toggle="season"' : 'data-tv-accordion-toggle="panel"';
  const seasonAttrs = scope === 'season' ? ' data-season-number="' + seasonNumber + '"' : '';
  const clickHandler =
    scope === 'season'
      ? 'event.preventDefault(); event.stopPropagation(); toggleTvSeasonDetails(' +
        requestId +
        ', ' +
        seasonNumber +
        ');'
...
function renderScopeChip(requestId, value, label, count, activeScope) {
  ...
  return (
    '<button type="button" onclick="setDetailsScope(' +
    requestId +
    ", '" +
    value +
    '\')" class="' +
...
function renderSeasonAccordion(data) {
  ...
  Object.values(tvInfo.releases_by_season || {}).forEach(function (releases) {
    (releases || []).forEach(function (release) {
      ...
      const isCompleteSeries =
        release.is_complete_series || release.covers_all_known_seasons || relScope.type === 'complete_series';
  ...
  tvInfo.seasons.forEach(function (season) {
    ...
    const packs = seasonReleases.filter(function (release) {
      return !isMultiSeasonRelease(release);
    });
  ...
  const scopeChipBar =
    '<div class="shrink-0 flex items-center gap-2.5 flex-wrap pb-4 mb-1 border-b border-gray-700/40">' +
  ...
  const seasonAccordion = tvInfo.seasons
    .map(function (season) {
      ...
      const packsChip = seasonPacks.length
        ? '<span class="inline-flex items-center gap-1 rounded-full border border-brand-500/30 bg-brand-600/10 px-2 py-0.5 text-[11px] text-brand-300">' +
          seasonPacks.length +
          ' pack' +
          (seasonPacks.length === 1 ? '' : 's') +
          '</span>'
        : '';
      const hasMarkable = (season.episodes || []).some(function (ep) {
        return !isEpisodeComplete(ep);
      });
      const seasonHasStaged = (season.episodes || []).some(function (ep) {
        return ep.status === 'staged';
      });
      ...
      const seasonLinks =
        '<div class="flex flex-wrap items-center gap-5 pl-7 text-xs">' +
        (hasMarkable
          ? '<button type="button" onclick="markSeasonAvailable(' +
            requestId +
            ', ' +
            season.id +
            '); event.stopPropagation();" class="text-brand-400 hover:text-brand-300">Mark all</button>'
          : '') +
        '<button type="button" onclick="stageIndividualEpisodes(this, ' +
        requestId +
        ', ' +
        season.season_number +
        '); event.preventDefault(); event.stopPropagation();" class="text-brand-400 hover:text-brand-300">Stage individual episodes</button>' +
        '</div>';

      const episodeHtml = (season.episodes || [])
        .map(function (ep) {
          ...
          const episodeDetailsId =
            'episode-details-' + requestId + '-' + season.season_number + '-' + ep.episode_number;
          ...
          const cachedEpisodeRowsHtml = episodeReleases.length
            ? episodeReleases
                .map(function (r) {
                  return renderReleaseCard(r, requestId, { bucket: true });
                })
                .join('')
          ...
          const coveringPack =
            stagedTorrent && (stagedTorrent.target_scope || {}).type !== 'single_episode' ? stagedTorrent : null;
          ...
          return (
            ...
            (isStaged ? 'bg-cyan-950/40' : 'open:bg-surface-900/60') +
            '" ontoggle="window.updateTvAccordionControls && window.updateTvAccordionControls()"' +
            (isOpen ? ' open' : '') +
            '>' +
            ...
            '<svg class="accordion-chevron w-3.5 h-3.5 text-gray-500 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>' +
            '<span class="text-xs font-mono text-gray-500 shrink-0">S' +
            ...
            (showInlineActions
              ? '<button type="button" onclick="markEpisodeAvailable(' +
                requestId +
                ', ' +
                ep.id +
                '); event.stopPropagation();" class="shrink-0 text-xs text-brand-400 hover:text-brand-300">Mark Available</button>'
              : '') +
            '</div>' +
            '</summary>' +
            ...
            '-' +
            ep.episode_number +
            '" class="pl-8 pr-3 pb-3 pt-0.5 space-y-1.5">' +
            episodeBucketHtml +
            '</div>' +
            '</details>'
          );
        ...
        .join('');
      const episodeListHtml = episodeHtml ? '<div class="divide-y divide-gray-700/40">' + episodeHtml + '</div>' : '';

      return (
        '<details id="season-details-' +
        requestId +
        '-' +
        season.season_number +
        '" class="group rounded-xl border border-gray-700/60 bg-surface-850" ontoggle="window.updateTvAccordionControls && window.updateTvAccordionControls()"' +
        (seasonHasStaged ? ' open' : '') +
        '>' +
        ...
        '<span class="text-white font-medium text-[15px]">Season ' +
        season.season_number +
        '</span>' +
        ...
        renderIconAction(
          'search',
          'Search Season ' + season.season_number + ' again',
          ' onclick="searchSeasonPacks(' +
            requestId +
            ', ' +
            season.season_number +
            '); event.preventDefault(); event.stopPropagation();"',
        ...
        seasonLinks +
        '</summary>' +
        '<div class="px-4 pb-5 pt-4 lg:px-5 space-y-2.5 border-t border-gray-700/40">' +
        renderSeasonPacksDrawer(requestId, season, seasonPacks, seasonStagedPacks) +
        episodeListHtml +
        '</div>' +
        '</details>'
      );
  ...
  const allSection =
    '<div class="space-y-3.5"' + (scope === 'all' ? '' : ' hidden') + '>' + panelToggle + seasonAccordion + '</div>';

  // ── Section: Season packs (per-season groups + multi-season group) ──
  const packsHelperRow =
    ...
    '<button type="button" onclick="searchAllSeasonPacks(' +
    requestId +
    ', this)" class="rounded-md border border-gray-600/80 bg-surface-900/70 px-2.5 py-1 text-xs font-medium text-gray-200 transition-colors hover:border-brand-400/70 hover:bg-surface-800 hover:text-white">Search all seasons</button>' +
    '</div>';
  const seasonPackGroups = tvInfo.seasons
    .map(function (season) {
      return renderSeasonPackGroup(
        requestId,
        season,
        packsBySeason[season.season_number] || [],
        stagedPacksForSeason(data.active_staged_torrents, season.season_number),
      );
    })
    .join('');
  const seasonPacksSection =
    '<div id="scope-season-packs-' +
    requestId +
    '" class="space-y-3"' +
    (scope === 'season_packs' ? '' : ' hidden') +
    '>' +
    packsHelperRow +
    seasonPackGroups +
  ...
  const completeSeriesHelperRow =
    '<div class="flex flex-wrap items-center justify-between gap-2">' +
    '<span class="text-xs text-gray-500">Packs covering every season. Searching also refreshes the multi-season pack group.</span>' +
    '<button type="button" onclick="searchMultiSeasonPacks(' +
    requestId +
    ')" class="rounded-md border border-gray-600/80 bg-surface-900/70 px-2.5 py-1 text-xs font-medium text-gray-200 transition-colors hover:border-brand-400/70 hover:bg-surface-800 hover:text-white">Search complete series</button>' +
    '</div>';
  const completeSeriesSection =
    '<div id="scope-complete-series-' +
    requestId +
    '" class="space-y-2"' +
    (scope === 'complete_series' ? '' : ' hidden') +
    '>' +
    ...
    (completeSeriesReleases.length
      ? '<ul class="divide-y divide-gray-700/40 rounded-xl border border-gray-700/60 bg-surface-850">' +
        completeSeriesReleases
          .map(function (r) {
            return renderReleaseCard(r, requestId);
          })
          .join('') +
        '</ul>'
...
async function markEpisodeAvailable(requestId, episodeId) {
  try {
    var response = await fetch('/requests/' + requestId + '/episodes/' + episodeId + '/mark-available', {
      method: 'POST',
    });
...
async function markSeasonAvailable(requestId, seasonId) {
  try {
    var response = await fetch('/requests/' + requestId + '/seasons/' + seasonId + '/mark-all-available', {
      method: 'POST',
    });
    if (!response.ok) throw new Error('Server error: ' + response.status);
    window.openRequestDetails(requestId);
...
function searchSeasonPacks(requestId, seasonNumber) {
  return new Promise(function (resolve) {
    ...
    containers.forEach(function (c) {
      c.innerHTML = renderSearchLoadingState('Searching season packs...');
    });
    window.startTvSearchProgress(
      '/requests/' + requestId + '/seasons/' + seasonNumber + '/season-packs/search/stream',
      'Season ' + seasonNumber + ' packs',
      async function (data) {
        var releases = (data.releases || []).filter(function (r) {
          return !isMultiSeasonRelease(r);
        });
...
function searchMultiSeasonPacks(requestId) {
  return new Promise(function (resolve) {
    ...
    if (!containers.length) return resolve();
    containers.forEach(function (c) {
      c.innerHTML = renderSearchLoadingState('Searching multi-season packs...');
    });
    window.startTvSearchProgress(
      '/requests/' + requestId + '/multi-season-packs/search/stream',
      'Multi-season packs',
      async function (data) {
...
async function searchEpisode(requestId, seasonNumber, episodeNumber) {
  ...
  window.startTvSearchProgress(
    '/requests/' + requestId + '/seasons/' + seasonNumber + '/episodes/' + episodeNumber + '/search/stream',
    episodeLabel,
    async function (data) {
      container.innerHTML =
        (data.releases || [])
          ...
          .join('') || '<div class="text-gray-500 text-sm py-2">No results found for this episode.</div>';
...
function tvDetailsNodesForRequest(requestId) {
  return Array.from(
    document.querySelectorAll(
      '#request-details-releases details[id^="season-details-' +
        requestId +
        '-"], #request-details-releases details[id^="episode-details-' +
        requestId +
        '-"], #request-details-releases details[id^="season-packs-details-' +
        requestId +
        '-"]',
    ),
  );
...
function toggleTvDetailsAll(requestId) {
  ...
  var shouldOpen = nodes.some(function (details) {
    return !details.open;
  });
...
function focusTvEpisode(requestId, seasonNumber, episodeNumber) {
  ...
  const targetDetails = document.getElementById(
    'episode-details-' + requestId + '-' + seasonNumber + '-' + episodeNumber,
  );
  if (!seasonDetails || !targetDetails) return;
  document
    .querySelectorAll('#request-details-releases details[id^="episode-details-' + requestId + '-"]')
