...
class PlexServiceLibraryScan:
    ...
    async def _get_library_sections_metadata(
        ...
        *,
        ...
        cached = self._cache.get_cached_sections(media_type)
        if cached is not None:
            return cached

        endpoint = f"{self._service.base_url}/library/sections"
        ...
        try:
            ...
            sections = [
                ...
                if (normalized := self._cache._normalize_section_metadata(section)) is not None
                and normalized["type"] == media_type
            ]
            self._cache.set_cached_sections(media_type, sections)
    ...
    async def iter_section_items(
        ...
        *,
        ...
        endpoint = (
            f"{self._service.base_url}"
            f"{self._cache._section_scan_endpoint(section_key, recently_added=recently_added)}"
        )
        client = await self._service._get_client()
        offset = 0
        ...
        while True:
            ...
            container = data.get("MediaContainer", {})
            items = self._cache._extract_metadata_items(container)
            for item in items:
                normalized = self._cache._normalize_library_item(item, section_key=section_key)
                if normalized is None:
                    continue
                self._cache._cache_item(normalized)
                yield normalized
    ...
    async def iter_library_items(
        ...
        *,
        ...
        for section in sections:
            async for item in self._service.iter_section_items(
                section["key"],
    ...
    async def get_all_show_rating_keys(self) -> list[str]:
        ...
        async for item in self._service.iter_full_library_items("show"):
    ...
    async def _get_tv_library_sections(self) -> list[str]:
        ...
        sections = await self._service._get_library_sections_metadata("show", strict=False)
    ...
    async def _get_movie_library_sections(self) -> list[str]:
        ...
        sections = await self._service._get_library_sections_metadata("movie", strict=False)
        return [str(section["key"]) for section in sections]
    ...
    async def _get_section_shows(self, section_key: str) -> list[str]:
        ...
        rating_keys: list[str] = []
        try:
            async for item in self._service.iter_section_items(section_key):
                if item.get("type") == "show" and item.get("rating_key"):
                    rating_keys.append(str(item["rating_key"]))
        except PlexTransientScanError:
            return []
