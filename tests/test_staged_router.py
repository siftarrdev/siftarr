...
class TestStagedRouter:
    ...
    @pytest.mark.asyncio
    async def test_bulk_approve_uses_qbit_bulk_add_for_more_than_seventeen(
        ...
        for index in range(18):
            torrent = MagicMock()
            torrent.id = index + 1
            torrent.request_id = 100 + index
            torrent.magnet_url = f"magnet:?xt=urn:btih:{index + 1:040x}"
            torrent.status = "staged"
            torrent.selection_source = "rule"
            torrent.torrent_path = f"/tmp/{index}.torrent"
            torrent.json_path = f"/tmp/{index}.json"
            torrent.title = f"Release {index}"
        ...
        qbit.add_torrents_bulk.return_value = [
            staged.BulkAddResult(key=torrent.id, success=True, torrent_hash=f"{torrent.id:040x}")
        ...
        body = json.loads(bytes(response.body))  # type: ignore[arg-type]
        assert body["processed"] == 18
        qbit.add_torrents_bulk.assert_awaited_once()
    ...
    @pytest.mark.asyncio
    async def test_bulk_approve_uses_sidecar_download_url_when_file_missing(
        self, mock_db, monkeypatch, tmp_path
    ):
        """Bulk approval should fall back to sidecar download URL per item."""
        json_path = tmp_path / "bulk-sidecar.json"
        json_path.write_text(
            json.dumps({"release": {"download_url": "https://example.test/bulk.torrent"}})
        )
        torrent = MagicMock()
        torrent.id = 21
        torrent.request_id = 221
        torrent.magnet_url = None
        torrent.info_hash = None
        torrent.status = "staged"
        torrent.selection_source = "rule"
        torrent.torrent_path = str(tmp_path / "missing-bulk.torrent")
        torrent.json_path = str(json_path)
        torrent.title = "Bulk Sidecar Release"
        torrent_result = MagicMock()
        torrent_result.scalars.return_value.all.return_value = [torrent]
        request = MagicMock(id=221, media_type=MediaType.MOVIE, status=RequestStatus.STAGED)
        request_result = MagicMock()
        request_result.scalar_one_or_none.return_value = request
        rule_result = MagicMock()
        rule_result.scalars.return_value.first.return_value = torrent
        ...
        qbit = AsyncMock()
        qbit.add_torrents_bulk.return_value = [
            staged.BulkAddResult(key=21, success=True, torrent_hash=None)
        ]
        monkeypatch.setattr(staged, "get_settings", lambda: MagicMock())
        monkeypatch.setattr(staged, "QbittorrentService", MagicMock(return_value=qbit))
        monkeypatch.setattr(staged, "LifecycleService", MagicMock(return_value=AsyncMock()))
        ...
        assert body["failed"] == []
        payload = qbit.add_torrents_bulk.await_args.args[0][0]
        assert payload.magnet_uri == "https://example.test/bulk.torrent"
        assert payload.torrent_path is None
        assert torrent.status == "approved"

    @pytest.mark.asyncio
    async def test_bulk_approve_reports_no_submission_source(self, mock_db, monkeypatch, tmp_path):
        """Bulk approval should fail one item clearly when no qBit source exists."""
        torrent = MagicMock()
        torrent.id = 22
        torrent.request_id = 222
        torrent.magnet_url = None
        torrent.info_hash = None
        torrent.status = "staged"
        torrent.selection_source = "rule"
        torrent.torrent_path = str(tmp_path / "missing-no-source.torrent")
        torrent.json_path = str(tmp_path / "missing-no-source.json")
        torrent.title = "No Source Release"
        torrent_result = MagicMock()
        torrent_result.scalars.return_value.all.return_value = [torrent]
        request = MagicMock(id=222, media_type=MediaType.MOVIE, status=RequestStatus.STAGED)
        request_result = MagicMock()
        request_result.scalar_one_or_none.return_value = request
        rule_result = MagicMock()
        rule_result.scalars.return_value.first.return_value = torrent
        ...
        assert body["failed"] == [
            {
                "id": 22,
                "title": "No Source Release",
                "error": "No local torrent file, magnet URL, or sidecar download URL available",
            }
        ]
        qbit.add_torrents_bulk.assert_not_awaited()
    ...
    @pytest.mark.asyncio
    async def test_approve_returns_json_success_for_application_json(self, mock_db, monkeypatch):
        """Single approve should return a JSON payload for fetch clients."""
        torrent = MagicMock(id=14, request_id=15, magnet_url="magnet:?xt=urn:btih:abc")
        torrent.status = "staged"
        torrent.selection_source = "rule"
        torrent.torrent_path = "/tmp/json.torrent"
        torrent.json_path = "/tmp/json.json"
        torrent.title = "JSON Release"
        request = MagicMock(id=15, media_type=MediaType.MOVIE, status=RequestStatus.STAGED)
        torrent_result = MagicMock()
        torrent_result.scalar_one_or_none.return_value = torrent
        request_result = MagicMock()
        request_result.scalar_one_or_none.return_value = request
        ...
        qbit = AsyncMock()
        qbit.add_torrent.return_value = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
        lifecycle_service = AsyncMock()
        monkeypatch.setattr(staged, "get_settings", lambda: MagicMock())
        monkeypatch.setattr(staged, "QbittorrentService", MagicMock(return_value=qbit))
        monkeypatch.setattr(staged, "LifecycleService", MagicMock(return_value=lifecycle_service))
        ...
        assert body == {
            "status": "ok",
            "message": "Torrent approved successfully",
            "torrent_id": 14,
    ...
    @pytest.mark.asyncio
    async def test_approve_accepts_qbit_metadata_pending_response(self, mock_db, monkeypatch):
        """Single approve accepts qbittorrentapi pending metadata responses."""
        torrent = MagicMock(id=18, request_id=19, magnet_url="magnet:?xt=urn:btih:abc")
        torrent.info_hash = None
        torrent.status = "staged"
        torrent.selection_source = "rule"
        torrent.torrent_path = "/tmp/metadata.torrent"
        torrent.json_path = "/tmp/metadata.json"
        torrent.title = "Metadata Pending Release"
        request = MagicMock(id=19, media_type=MediaType.MOVIE, status=RequestStatus.STAGED)
        torrent_result = MagicMock()
        torrent_result.scalar_one_or_none.return_value = torrent
        request_result = MagicMock()
        request_result.scalar_one_or_none.return_value = request
    ...
    async def test_approve_qbit_failure_does_not_commit_local_transition(self, monkeypatch):
        """A true qBit add failure should not persist approved/downloading state."""
        ...
        async with session_maker() as db:
            ...
            torrent = StagedTorrent(
                ...
                torrent_path="/tmp/failure.torrent",
                json_path="/tmp/failure.json",
                original_filename="failure.torrent",
                title="Failure Movie 2026",
                size=100,
                indexer="test",
                score=90,
                magnet_url="magnet:?xt=urn:btih:4444444444444444444444444444444444444444",
            ...
            qbit = AsyncMock()
            qbit.add_torrent.return_value = None
            qbit.get_torrent_info.return_value = None
            qbit.get_torrent_info_by_name.return_value = None
            monkeypatch.setattr(staged, "get_settings", lambda: MagicMock())
            monkeypatch.setattr(staged, "QbittorrentService", MagicMock(return_value=qbit))
            monkeypatch.setattr(staged, "log_staging_decision", MagicMock())
            monkeypatch.setattr(staged.os.path, "exists", MagicMock(return_value=False))
            ...
            assert request.status == RequestStatus.STAGED
            assert torrent.status == "staged"
...
class TestDownloadStatusEndpoint:
    ...
    @pytest.mark.asyncio
    async def test_download_status_keeps_torrent_for_pending_tv_request(self, mock_db, monkeypatch):
        """An approved TV torrent stays visible when the request derives PENDING.

        Regression test: mixed episode statuses derive the TV request back to
        PENDING while an approved episode torrent is still downloading; the
        download-status endpoint must keep reporting its progress.
        """
        import json
        ...
        torrent = MagicMock()
        torrent.id = 8
        torrent.title = "Example Show S01E01 1080p WEB-DL"
        torrent.request_id = 102
        torrent.magnet_url = "magnet:?xt=urn:btih:da39a3ee5e6b4b0d3255bfef95601890afd80709"
        torrent.status = "approved"
        ...
        request_status_result = MagicMock()
        request_status_result.all.return_value = [(102, RequestStatus.PENDING)]

        mock_db.execute.side_effect = [torrent_result, request_status_result]

        qbit = AsyncMock()
        qbit.get_torrent_info = AsyncMock(return_value={"progress": 0.4, "state": "downloading"})
        monkeypatch.setattr(staged, "get_settings", lambda: MagicMock())
        monkeypatch.setattr(staged_module, "QbittorrentService", MagicMock(return_value=qbit))
        ...
        assert response.status_code == 200
        body = json.loads(bytes(response.body))  # type: ignore[arg-type]
        assert [item["id"] for item in body["torrents"]] == [8]
        assert body["torrents"][0]["request_status"] == RequestStatus.PENDING.value
