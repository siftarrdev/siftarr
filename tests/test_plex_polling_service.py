...
class TestPlexPollingService:
    ...
    async def test_reconcile_request_tv_loads_request_and_reuses_episode_sync_path(
        ...
        """Targeted TV reconciliation should reload seasons/episodes and reuse episode sync."""
        ep1 = _make_episode(1)
        ep2 = _make_episode(2)
        season = _make_season(1, [ep1, ep2])
        req = _make_request(
    ...
    @pytest.mark.asyncio
    async def test_reconcile_request_tv_partial_availability(self, service, mock_db, mock_plex):
        """Targeted TV reconciliation should surface partial availability immediately."""
        ep1 = _make_episode(1)
        ep2 = _make_episode(2)
        season = _make_season(1, [ep1, ep2])
        req = _make_request(
    ...
    @pytest.mark.asyncio
    async def test_reconcile_request_tv_full_availability(self, service, mock_db, mock_plex):
        """Targeted TV reconciliation should report full availability when all episodes exist."""
        ep1 = _make_episode(1)
        ep2 = _make_episode(2)
        season = _make_season(1, [ep1, ep2])
        req = _make_request(
