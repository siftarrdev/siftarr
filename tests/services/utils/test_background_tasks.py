...
@pytest.mark.asyncio
async def test_background_episode_refresh_skips_after_sqlite_lock_retries():
    db = AsyncMock()
    session_context = AsyncMock()
    session_context.__aenter__.return_value = db

    episode_sync = AsyncMock()
    episode_sync.sync_request = AsyncMock(
        side_effect=[_sqlite_locked_error(), _sqlite_locked_error()]
    )
