...
class TestReleaseSelectionService:
    ...
    async def test_use_releases_tv_single_episode_selection_only_replaces_same_episode_stage(
        ...
        """TV single-episode manual reselection should not replace staged siblings."""
        settings = MagicMock(staging_mode_enabled=True)
        queue_service = AsyncMock()
        staging_service = AsyncMock()

        request_record = MagicMock(spec=Request)
        request_record.id = 9
        request_record.media_type = MediaType.TV
        request_record.status = RequestStatus.PENDING

        episode_one_release = MagicMock(spec=Release)
        ...
        episode_two_release = MagicMock(spec=Release)
        ...
        reselection_release = MagicMock(spec=Release)
    ...
    async def test_use_releases_tv_single_episode_reuses_same_episode_stage_without_replacing_sibling(
        ...
        """Reusing an already staged TV episode should leave other episode stages untouched."""
        settings = MagicMock(staging_mode_enabled=True)
        queue_service = AsyncMock()
        staging_service = AsyncMock()
        ...
        selected_release = MagicMock(spec=Release)
